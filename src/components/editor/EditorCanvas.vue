...
const TOOL_CURSORS = computed(() => ({
  brush: makeToolCursor(
    `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
      <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25z" fill="${props.selectedColor}" stroke="#fff" stroke-width="0.8"/>
      <path d="M20.71 5.63l-2.34-2.34a1 1 0 00-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83a1 1 0 000-1.41z" fill="${darken(props.selectedColor, 40)}" stroke="#fff" stroke-width="0.5"/>
    </svg>`, 2, 21
  ),
...
const canvasStyle = computed(() => {
  ...
  if (props.tool === 'move' || isPanning.value || spaceHeld.value) {
    ...
    cursor = TOOL_CURSORS.value.brush
  } else if (props.tool === 'eraser') {
    cursor = TOOL_CURSORS.value.eraser
  } else if (props.tool === 'picker') {
    cursor = TOOL_CURSORS.value.picker
  } else if (props.tool === 'fill') {
    cursor = TOOL_CURSORS.value.fill
  }
